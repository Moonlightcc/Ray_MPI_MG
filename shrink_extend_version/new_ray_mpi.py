...
RECONFIG_THRESHOLD = 100 # Number of ranks * number of iterations
...
class RayMPIRuntime:
    ...
    async def reconfigure_test(self, rank):
        ...
        if rank == 0:
            ...
            if self.checkpoint_count > RECONFIG_THRESHOLD:
    ...
    async def change_ranks(self, num_ranks: int):
        print("MPIRuntime: To change ranks from {} to {}".format(self.world_size, num_ranks))
    ...
    '''
        Scenarios that is function is called:
        1. Failure happens: This function should recover using the checkpoints, using the ranks that are still active
        2. self.should_change_rank is set by the user: This denote changes in compute resources, and this function should change the world size and redistribute data
        3. Checkpoint counter reaches threshold: This function simply checkpoint all the states for failure recovery
    '''
    async def reconfigure(self, rank: int, state_ref_l):
        ...
        if self.should_change_rank: # TODO:
            ...
            self.rank_states[rank] = state_ref_l

            # Redistribute data 

            # Change meta data, send_receive buffer / collective buffer
            self._metadata_update(self.target_rank)

            # Store new data to checkpoint
            self.rank_states[rank] = []
    ...
    def _metadata_update(self, new_num_ranks):
        self.world_size = new_num_ranks
        self.rank_states = [None] * self.world_size
        self.rank_active = [True] * self.world
        self.collective_buffer = [None] * self.world_size
        self.reborn_flag = [False] * self.world_size
    ...
    async def send(self, data, src_rank: int, dest_rank: int):
        ...
        print("MPIRuntime: Sending data from rank {} to rank {}".format(src_rank, dest_rank))
