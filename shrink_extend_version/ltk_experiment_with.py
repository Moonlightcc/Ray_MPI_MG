...
class mg_actor:
    ...
    def rprj3(self, r, grid_level):
        ...
        if index_of_active_process % 2 == 0:
            ...
            self.recorder.register_time.remote(self.rank, self.current_iteration, "rank_sleep", time.time())
            return None
    ...
    def interp(self,z,grid_level):
        ...
        if self.process_activate_flag == True and self.rank % coarse_process_step == 0:
            ...
            self.process_activate_flag = True
            self.recorder.register_time.remote(self.rank, self.current_iteration, "end_sleep", time.time())
    ...
    def mg3P(self,v,a,c):
        ...
        while self.current_level > 1:
            ...
            if self.rank % (2 ** self.current_level) == 0:
                ...
                self.recorder.register_time.remote(dest_rank, self.current_iteration, "recovery_start", time.time())
                ray.get(self.MPIRuntime.expand_rank.remote(dest_rank))
            if self.rank % (2 ** self.current_level) != 0 and self.rank % (2 ** (self.current_level-1)) == 0:
                self.recorder.register_time.remote(self.rank, self.current_iteration, "recovery_end", time.time())
            ray.get(self.MPIRuntime.barrier_with_target_num.remote(self.rank, self.current_active_process * 2))
    ...
    def shrink_rank(self, current_world_size):
        self.update_state()
        state_ref = ray.put(self.state, _owner=self.MPIRuntime)
        self.recorder.register_time.remote(self.rank, self.current_iteration, "delete_start", time.time())
        ray.get(self.MPIRuntime.shrink_rank_with_record.remote(self.rank, [state_ref], self.current_iteration, True, current_world_size))
        # ray.get(self.MPIRuntime.shrink_rank.remote(self.rank, [state_ref], True, current_world_size))
    ...
    def main(self):
        ...
        if self.rank == 0:
            ...
            ray.get(self.MPIRuntime.get_time_recorder.remote())

        while self.current_iteration < self.iteration_number:
            self.recorder.register_time.remote(self.rank, self.current_iteration, "iteration_start", time.time())
            ...
            # if False and self.current_iteration != 0 and self.current_iteration % phase_length == 0:
            ...
            self.recorder.register_time.remote(self.rank, self.current_iteration, "iteration_end", time.time())
